...
<div class="container" style=" align-content: center; padding-top:40px;">
   ...
   <div id="result" style="padding-left: 140px; align-content: center;">
        ...
        <div class="shadow-4 rounded-5 overflow-hidden">
            <table id="example" class="table bg-white table-hover table-active-bg-factor table-bordered" style="width: 85%;">
                ...
                <tbody>
                  ...
                  $sql = "select * from admin order by name";//ORDER BY id desc
         ...
         if(mysqli_num_rows($result)>0 )
                {
                    ...
                    <tr>
                        ...
                        <td>
                         ...
                         <td><a href="viewadmin.php?del=<?php echo $row["a_id"];?>" class="btn btn-primary" style="text-align: right; font-family: cursive" class="btn text-danger">Delete</a></td>
                          <td><a href="updateadmininfo.php?edi=<?php echo  $row["a_id"];?>" class="btn btn-primary" style="text-align: right; font-family: cursive" class="btn text-danger">Update</a></td>
    ...
    if(isset($_GET['del'])){
    $id = $_GET['del']; 
    $sql1 = "select user from admin where a_id= '$id'";
    $query = mysqli_query($con,$sql1); 
    $row = mysqli_fetch_assoc($query);
        $user=$row["user"];
        if(quiery1)
        {
            $sql2 = "Delete from admin where user= '$user'";
    $sql3 = "Delete from user where u_name=$user";
    $query2 = mysqli_query($con,$sql2);
    $query3 = mysqli_query($con,$sql3);
         if($query3 && $query2 ){
        echo "<script type='text/javascript'> alert('Admin is deleted')</script>";
...
<div class="form-popup" id="myForm">
     <div>
       <form class="form-container" method="POST" action="viewadmin.php" enctype="multipart/form-data">
    ...
    $a_idd=0;
    if(strlen($phone)==11){
          $c = "INSERT INTO `admin`(`name`, `address`, `phone`, `email`,`image`, `gender`, `token`, `status`,`user`,`password`,`a_idd`) VALUES ('$name','$address','$phone','$email','$imagename','$gender','$token','$status','0','0','$a_idd')";
          $sql3 = "INSERT INTO `user`(`u_type`,`token`,`status`,`u_name`,`u_password`,`email`) VALUES ('$type','$token','$status','0','0','$email')"; 
           ...
           $query3 = mysqli_query($con,$sql3);
            ...
            $body = "Hi,$name. click on the link to active your account http://localhost/CRM/customer/confirmcode/Super%20Admin/adminuserpass.php?token=$token";
