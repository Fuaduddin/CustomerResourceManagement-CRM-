...
if(isset($_POST['add'])){
    ...
    if(strlen($phone)==11){
          $c = "INSERT INTO `staff`(`name`, `address`, `phone`, `email`,`image`, `gender`, `token`, `status`,`user`,`password`) VALUES ('$name','$address','$phone','$email','$imagename','$gender','$token','$status','0','0')";
          $sql3 = "INSERT INTO `user`(`type`,`token`,`status`,`email`,`password`) VALUES ('$type','$token','$status','$email','0','0')"; 
           $query2 = mysqli_query($con,$c);
           $query3 = mysqli_query($con,$sql3);
        
        if($query2 & $query3){
            $subject = "Account Activation";
            $body = "Hi,$name. click on the link to active your account http://localhost/CRM/customer/customeruserpass.php?token=$token";
