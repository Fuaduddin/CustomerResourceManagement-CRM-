                ...
                <li>
                 ...
                 <ul class="feat-show">
                   ...
                   while ($row = mysqli_fetch_assoc($query1)){
                       $pid=$row['p_id'];
                       echo " <li style='font-size: 18px;  color: white; text-transform:uppercase;'><a href='products.php?id=$pid'></li>" .$row["type"]."</a></li>";
                   }
                        ?>
                 </ul>
              </li>
...
$row = mysqli_fetch_assoc($query);  
                    $email=$row["email"];
                  $sql = "select * from `appointment` where c_email='$email' order by ap_id";//ORDER BY id desc
         ...
         if(mysqli_num_rows($result)>0 )
                {
                    ...
                    <tr>
                        <td><?php echo $row["ap_id"];?></td>
                         <td> Product : <?php echo $row["product"];?> 
                         Product Category: <?php echo $row["productcategory"];?> </td>
